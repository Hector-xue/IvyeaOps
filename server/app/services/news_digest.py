...
# Anthropic / Marketplace Pulse feeds are gone (404) — replaced with live feeds
# verified 2026-07: Google AI / HuggingFace / MIT TR, ChannelX / RetailDive / SellerLabs.
...
async def generate_digest(target_date: Optional[str] = None) -> dict[str, Any]:
    ...
    # De-dup by URL, then cap the batch sent to the model. Interleave the two
    # categories before capping — a straight [:25] in feed order starved
    # amazon_seller down to 1-2 items because seller feeds are listed last.
    seen: set[str] = set()
    ...
    queues = [by_cat[c] for c in ("ai_industry", "amazon_seller") if by_cat.get(c)]
    while len(inputs) < 25 and queues:
    ...
    if not inputs:
        ...
        try:
            # agent_retrieval=False: knowledge-base citations would corrupt the
            # strict-JSON answer this task needs. Output occasionally comes back
            # truncated/unparseable — retry once before falling back to raw text.
            for _attempt in range(2):
                _prov, text = await ai_synthesis_service.run_text_chain(
                    _build_prompt(inputs), agent_retrieval=False
                )
